...
export const useServiceCreateProduct = () => {
  ...
  return useMutation<TResponse, Error, CreateProductForm>({
    mutationFn: async (data: CreateProductForm) => {
      ...
      formData.append("realSellingPrice", data.realSellingPrice.toString());
      if (data.listedPrice !== null) {
        formData.append("listedPrice", data.listedPrice.toString());
      }
      formData.append("totalStockQuantity", data.totalStockQuantity.toString());
