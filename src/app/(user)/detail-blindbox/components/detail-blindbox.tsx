...
export default function BlindboxDetail({ blindBoxId }: BlindboxProps) {
  ...
  const { addBlindboxToCartApi, isPending: isAddingToCart } =
    useAddBlindboxToCart();
  const { getPSellerByIdApi, isPending: isSellerPending } = useGetSellerById();
  const { getOverViewSellerApi, isPending: isSellerOverviewPending } =
    useGetOverviewSeller();
        ...
        >
          ...
          <div className="flex gap-8">
            <div className="flex gap-8">
              <p className="text-xl">
                Thương hiệu:{" "}
                <span className="text-[#00579D] uppercase">
                  {blindbox?.brand}
                </span>
              </p>
              ...
              <p className="text-xl">
                Tình trạng:{" "}
                <span className="text-[#00579D]">
                  {stockStatusMap[blindbox?.blindBoxStockStatus as StockStatus]}{" "}
                  ({blindbox?.totalQuantity})
                </span>
              </p>
              <div className="w-px h-5 bg-gray-300" />
            </div>
          ...
          {isReleased && (
            <div className="flex items-center gap-4 mt-6">
              ...
              <button
                ...
                disabled={
                  isAddingToCart || !blindbox || blindbox?.totalQuantity === 0
                }
          ...
          <div className="md:flex-row items-center gap-8 flex flex-col">
            <div className="relative w-20 h-20 flex-shrink-0">
              ...
              <div
                ...
                className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 bg-red-500 text-white text-xs px-2 py-0.5 rounded-sm shadow-sm whitespace-nowrap text-center max-w-[120px] truncate"
              >
                {sellerInfo?.fullName}
              </div>
            </div>
          ...
          <div className="flex-1">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4 text-sm sm:text-base text-gray-600">
              <div className="flex justify-between sm:justify-start sm:gap-4">
                <p>Đánh Giá</p>
                <p className="text-red-600 font-semibold">
                  {isSellerOverviewPending ? "..." : seller?.averageRating}
                </p>
              </div>
              <div className="flex justify-between sm:justify-start sm:gap-4">
                <p>Tham Gia</p>
                <p className="text-red-600 font-semibold">
                  {isSellerOverviewPending
                    ? "..."
                    : seller?.joinedAt
                    ? format(new Date(seller.joinedAt), "dd-MM-yyyy", {
                        locale: vi,
                      })
                    : "Chưa rõ"}
                </p>
              ...
              <div className="flex justify-between sm:justify-start sm:gap-4">
                <p>Sản Phẩm</p>
                <p className="text-red-600 font-semibold">
                  {isSellerOverviewPending || isLoadingProducts
                    ? "..."
                    : seller?.productInSellingCount}
                </p>
              </div>
              <div className="flex justify-between sm:justify-start sm:gap-4">
                <p>Blindbox</p>
                <p className="text-red-600 font-semibold">
                  {isSellerOverviewPending || isLoadingProducts
                    ? "..."
                    : seller?.blindBoxCount}
                </p>
              </div>
              <div className="flex justify-between sm:justify-start sm:gap-4">
                <p>Công Ty</p>
                <p className="text-red-600 font-semibold">
                  {isSellerOverviewPending ? "..." : seller?.companyName}
                </p>
              </div>
              <div className="flex justify-between sm:justify-start sm:gap-4">
                <p>Khu Vực</p>
                <p className="text-red-600 font-semibold">
                  {isSellerOverviewPending ? "..." : seller?.companyArea}
                </p>
              </div>
            </div>
          </div>
