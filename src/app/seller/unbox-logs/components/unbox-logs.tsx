...
import {
  ...
  ResponseUnboxLogs,
  ResponseUnboxLogsList,
} from "@/services/unbox/typings";
...
export default function UnboxLogs() {
  ...
  const { isPending: isLoadingProduct, getProductByIdWebApi } =
    useGetProductByIdWeb();
  const [logs, setLogs] = useState<ResponseUnboxLogs[]>([]);
  const [selectedLog, setSelectedLog] = useState<ResponseUnboxLogs | null>(
    null
  );
  const [productDetail, setProductDetail] = useState<AllProduct | null>(null);
  const [paginationData, setPaginationData] =
    useState<ResponseUnboxLogsList | null>(null);
        ...
        <CardContent className="space-y-6 py-6">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-semibold">
              Danh sách khách trúng thưởng blindbox
            </h2>
          </div>
              ...
              {isPending
                ...
                : logs.map((log) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>
                        <span
                          className={`px-2 py-1 rounded text-xs font-medium text-center ${
                            RarityColorClass[log.rarity as Rarity]
                          }`}
                        >
                      ...
                      <TableCell>
                        {moment(log.unboxedAt).format("DD/MM/YYYY")}
                      </TableCell>
                      <TableCell>
                        <Button
                          variant="outline"
